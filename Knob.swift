...
class Knob: UIControl {
    ...
    var isContinuous = true //the control calls back repeatedly as the value changes. if false the contorl calls back once after the user has finished interacting with it
...
class KnobRenderer {
    ...
    //CGAffineTransform uses a 3x3 matrix and CATransform3D uses a 4x4 matrix; the addition of the z-axis requires the extra values. At their core, 3D transformations are simply matrix multiplications.
    ...
    private func updateTrackLayerPath() { //creates an arc between startAngle and endAngle values with a radius that ensures the pointer will fit within the layer, and positions it on the center of the trackLayer.
    ...
    private func updatePointerLayerPath() { //creates the path for the pointer at the poisition where angle is equal to zero. Since the pointer is a straight line, all you need to draw the pointer are move(to:) and addLine(to:)
        let bounds = trackLayer.bounds
